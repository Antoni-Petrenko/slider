...
export function createSlide(data,targetElement){
    ...
    slideImgContainer=createElement('div',"slides","slider__img-container")(targetElement);
    products.forEach((product,productID)=>{
        ...
        product.productIMG.forEach((src,dataID)=>{
          createElement('img',null,'product-container__product',src,productName[productID],dataID)(productContainer)
        })
  ...
  export function createSlideControl(data,targetElement){
    const productName=Object.keys(data),
    products=Object.values(data),
    slideControlsContainer=createElement('div',null,"controls-container")(targetElement);
    
    
    products.forEach((product,productID)=>{
     const control=createElement('div',null,"control")(slideControlsContainer),
      controlButtons=createElement('div',null,"control-buttons")(control)
      createElement('img',null,"shadow--control-section","./asset/img/P_shadow.png","shadow",null)(control)
      
      product.buttons.forEach((button,dataID)=>{
        createButton(productName[productID],dataID,button,changeColor)(controlButtons);
      })
      product.info.forEach((el,productID)=>{
        const info=Object.entries(el);
        createElement("img",null,info[0][0],info[0][1],productName[productID])(control);
